...
>
> 最后更新：2026-06-14 by Claude（分支 `feat/forecasting-engine-polish2` → **PR #18 已合并 main** `fbfe14a`；typecheck + `next build` + vitest 全绿）。本轮：**Forecasting Engine 公开控制台 `/research` 全量双语化**（EN 默认匹配 apex + 一键 `中文` toggle，chrome 与流式研究内容**一起**切换）。做法：新增 `apps/web/lib/research/locale.ts`（叶子 locale 类型 + `pick`）+ `i18n.ts`（chrome 字典 + `stanceLabel`），把 `locale` 从 composer → SSE POST body → route → driver → `buildPredictionDemoRun`/`replayRun` 全链路打通，**服务端生成的内容**（判词/证据/叙述/阶段）按语言生成，而非只换 UI 标签；伊朗(真实快照)+美联储两个 curated case 的 EN 文本忠实翻译，**数字/日期/单位/URL/专有名词全部保留**。console 现自持完整 shell（header + 语言开关 + footer），`page.tsx` 变薄；旧 `/api/prediction-engine/run` 默认 zh 保持原行为。一并收尾 forecaster review 三点：研究快照日期+距截止天数更醒目、"80% 主观可信区间"措辞 + 说明、edge caveat。**已部署到 forecasting-agent.com**（web 项目 `prj_kPZRC…`，`scripts/world-cup/deploy-web.sh`，dpl `CK6Pn27…` Ready+production），实测 `/research` EN/zh 双语 + toggle live 工作、0 console error、world-cup apex 未受影响——**已解决上一会话"线上仍是旧版"的部署阻塞**。/research beta 入口仍 dormant(注释)。已清理 2 个 worktree(cend/merge) + drop 掉已被 main 取代的 stash(world-cup [locale] WIP，opencc-js 等均已在 main)。⚠️ 见下 P2：`deploy-web.sh` promote 步在新版 vercel CLI 下有 bug。此前(2026-06-12)：pulse:*→forecast:* 改名；MC 淘汰赛点球规则偏高待用户拍板(见下"MC 淘汰赛模型核查")；catch-all 路由部署需补 check:true）
