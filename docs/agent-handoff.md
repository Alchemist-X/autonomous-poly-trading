...
> ④ **部署**：clean-untar（备份 `~/predict-raven.pre-report-quality.bak`）+ 回拷 .env/override，`docker compose build raven` 后 `up -d --no-deps paper-agent`——**镜像漂移现状：paper-agent 现跑本分支（=main+PR #81）；raven 在 7-06 main；仍旧镜像的只剩 forecast-api**。⚠️ PR #81 未合 main 前，任何人重建镜像会把 paper-agent 回退掉这批修复。
> ⑤ **待办**：PR #81 等 CI + review 后合并；limit fill-rate 按事件计数 >100%（应按 distinct limitId，旧缺陷）；存量英文 dossier 续跑会中英混排（一次性过渡）；首批市场结算后（Khamenei 7/15、Hormuz 7/31）看首个真实 skill score。英文版 handoff 此条待同步翻译。
>
