...
如果按"价值 / 阻塞 / 工期"打分，下一步排序：

| 顺序 | 动作 | 工期 | 阻塞依赖 |
| --- | --- | --- | --- |
| 1 | **Mode A（Pure Managed AI）开始 Phase 3a 实现** | 4-6 周 | 无（Phase 2 已铺好） |
| 2 | 用户拍板 Mode A 3 个子决策（session scope / approval gate / 6 月只靠 rewards 是否 OK） | 5min 决定 | 无 |
| 3 | **Computer Use D1（UMA 仲裁监控）pilot** | 1 周 | 无（read-only 工具，与主线并行） |
| 4 | **Builder Code Task #6（执行器 wire builder code，underdog 测试）** | 10-15min agent | 无 |
| 5 | Computer Use D2（Privy onboarding QA on testnet） | 1 周 | testnet 环境 ready |
